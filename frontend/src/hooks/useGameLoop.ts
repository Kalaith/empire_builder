...
export const useGameLoop = () => {
  ...
  const moveHeroTowards = useCallback(
    (hero: Hero, targetX: number, targetY: number) => {
  ...
  const moveEnemyTowards = useCallback(
  ...
  const getPositionBonus = useCallback(
    (attacker: Hero, defender: Enemy) => {
      let bonus = 0;
  ...
  const updateHeroAI = useCallback(
    (hero: Hero) => {
