...
const GameGrid: React.FC = () => {
  const { grid, gridWidth, gridHeight, placeBuilding, placeFlag, resources } =
    useGameStore();
  const {
  ...
  const handleFlagSelect = (type: string) => {
    if (type === "cancel") {
      ...
      selectFlagType(type);
      addGameMessage(
        `Selected ${flagTypes[type].name} - click empty cell to place`,
        "info",
      );
    }
  };
  ...
  const renderCell = (cell: GridCell, x: number, y: number) => {
    let cellContent = "";
    let cellClass =
      "grid-cell w-8 h-8 border border-gray-300 cursor-pointer hover:bg-gray-100 flex items-center justify-center text-sm";
          ...
          {Object.entries(flagTypes).map(([type, flag]) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`flag-cost text-xs ml-1 ${canAfford ? "text-green-600" : "text-red-500"}`}
                >
                  {flag.baseCost}g
                </span>
              </button>
