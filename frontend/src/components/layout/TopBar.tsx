...
const TopBar: React.FC = () => {
  const { resources, heroes, buildings, enemies, isPaused, gameTime, restartGame } = useGameStore();
  ...
  useEffect(() => {
    ...
    if (prevGold > 0 && goldChange !== 0) {
      setResourceChanges(prev => ({ ...prev, goldChange, gold: resources.gold }));
      setTimeout(() => {
    ...
    <div className="top-bar bg-white rounded-lg shadow-md p-4 mb-4">
      <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            {isPaused && (
              ...
              >
                ...
                <span className="text-orange-700 font-medium text-sm">GAME PAUSED</span>
              </motion.div>
            )}
          </div>
        ...
        <div className="resource-display flex gap-4 justify-center flex-wrap">
          {[
            { key: 'gold', icon: '💰', value: resources.gold, color: 'yellow', max: null },
            { key: 'mana', icon: '🔮', value: resources.mana, color: 'blue', max: null },
            { key: 'supplies', icon: '📦', value: resources.supplies, color: 'green', max: null },
            { key: 'population', icon: '👥', value: resources.population, color: 'purple', max: resources.maxPopulation }
          ].map(resource => (
            <motion.div
              key={resource.key}
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <span className={`stat-icon text-${resource.color}-500 text-xl`}>{resource.icon}</span>
                <span className="stat-label font-medium text-sm capitalize text-gray-700">{resource.key}:</span>
              </div>
              <div className={`stat-value text-xl font-bold text-${resource.color}-700 relative`}>
                {Math.floor(resource.value)}
                {resource.max !== null && `/${resource.max}`}
                <AnimatePresence>
                  {resource.key === 'gold' && resourceChanges.goldChange > 0 && (
                    <motion.span
              ...
              <div className="game-stats bg-gray-50 rounded-lg p-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div className="stat-item bg-white rounded-lg p-3 flex flex-col items-center">
                    ...
                    <span className="stat-value text-2xl font-bold text-blue-600">{heroes.length}</span>
                    {heroes.length > 0 && (
                      <div className="text-xs text-gray-500 mt-1">
                        Avg Level: {Math.round(heroes.reduce((sum, h) => sum + h.level, 0) / heroes.length)}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="stat-item bg-white rounded-lg p-3 flex flex-col items-center">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="stat-icon text-amber-500 text-lg">🏠</span>
                      <span className="stat-label font-medium">Buildings</span>
                    </div>
                    <span className="stat-value text-2xl font-bold text-amber-600">{buildings.length}</span>
                    <div className="text-xs text-gray-500 mt-1">
                      Total Income: +{buildings.reduce((sum, b) => sum + (b.production.gold || 0), 0)}/min
                    </div>
                  </div>
                  ...
                  <div className="stat-item bg-white rounded-lg p-3 flex flex-col items-center">
                    ...
                    <span className={`stat-value text-2xl font-bold ${
                      enemies.length === 0 ? 'text-green-600' :
                      enemies.length < 3 ? 'text-yellow-600' : 'text-red-600'
                    }`}>
                      {enemies.length}
                    </span>
                    <div className={`text-xs mt-1 ${
                      enemies.length === 0 ? 'text-green-500' :
                      enemies.length < 3 ? 'text-yellow-500' : 'text-red-500'
                    }`}>
                      {enemies.length === 0 ? 'Safe' : enemies.length < 3 ? 'Minor Threat' : 'Under Attack!'}
                    </div>
                  </div>

                  <div className="stat-item bg-white rounded-lg p-3 flex flex-col items-center">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="stat-icon text-gray-500 text-lg">⏰</span>
                      <span className="stat-label font-medium">Time</span>
                    </div>
                    <span className="stat-value text-2xl font-bold text-gray-600">{formatTime(gameTime)}</span>
                    <div className="text-xs text-gray-500 mt-1">
                      Session Time
                    </div>
                  </div>
