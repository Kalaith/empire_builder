...
const RightSidebar: React.FC = () => {
  ...
  const [activePanel, setActivePanel] = useState<'info' | 'quests' | 'achievements' | 'heroes' | 'saves'>('info');
  const [selectedHero, setSelectedHero] = useState<Hero | null>(null);
  const [selectedBuilding, setSelectedBuilding] = useState<Building | null>(null);
  const [saveLoadMenu, setSaveLoadMenu] = useState<{ isOpen: boolean; mode: 'save' | 'load' }>({ isOpen: false, mode: 'save' });
        ...
        return (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div><strong>Health:</strong> {building.healthPoints}/{building.maxHealthPoints}</div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-sm">
                <span><strong>Level {hero.level}</strong> {hero.className}</span>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-sm">
                <span><strong>Health:</strong> {hero.health}/{hero.maxHealth}</span>
                <span className={`text-xs ${
                  hero.health / hero.maxHealth > 0.7 ? 'text-green-600' :
                  hero.health / hero.maxHealth > 0.3 ? 'text-yellow-600' : 'text-red-600'
                }`}>
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${
                    hero.health / hero.maxHealth > 0.7 ? 'bg-green-500' :
                    hero.health / hero.maxHealth > 0.3 ? 'bg-yellow-500' : 'bg-red-500'
                  }`}
            ...
            {hero.specialization && (
              <div className="bg-purple-50 p-2 rounded">
                <div className="text-xs font-medium text-purple-800">{hero.specialization.name}</div>
      ...
      <div className="bg-white rounded-lg shadow-md p-2">
        <div className="flex gap-1">
          {[
            ...
            <button
              key={tab.id}
              onClick={() => setActivePanel(tab.id as 'info' | 'heroes' | 'quests' | 'achievements' | 'saves')}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="hero-details flex-1">
                        ...
                        <div className="hero-stats flex justify-between text-xs text-gray-600">
                          <span>HP: {hero.health}/{hero.maxHealth}</span>
                          <span>💰{hero.gold}</span>
