...
const QuestPanel: React.FC = () => {
  ...
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      ...
      case 'legendary': return 'text-red-600 bg-red-100';
        ...
        {filteredQuests.map(quest => (
          ...
          >
            ...
            <div className="objectives space-y-1">
              {quest.objectives.map(objective => (
                <div key={objective.id} className="flex justify-between items-center text-xs">
                  <span className={objective.isCompleted ? 'text-green-600 line-through' : 'text-gray-600'}>
                    {objective.description}
                  </span>
                  <span className={`font-medium ${objective.isCompleted ? 'text-green-600' : 'text-gray-500'}`}>
            ...
            <div className="rewards flex gap-2 mt-2 text-xs text-gray-600">
              <span>Rewards:</span>
              {quest.rewards.gold && <span className="text-yellow-600">💰{quest.rewards.gold}</span>}
              {quest.rewards.mana && <span className="text-blue-600">🔮{quest.rewards.mana}</span>}
              {quest.rewards.supplies && <span className="text-green-600">📦{quest.rewards.supplies}</span>}
            </div>
              ...
              >
                ...
                {quest.assignedHeroId && (
                  <div className="text-xs text-gray-600 mt-2">
                    Assigned to: {heroes.find(h => h.id === quest.assignedHeroId)?.heroName || 'Unknown Hero'}
                  </div>
                )}
              </motion.div>
            )}
